...
import { simulateNfcScan } from '../../data/mockData';
...
const NFCScanner: React.FC = () => {
  ...
  useEffect(() => {
    if (isScanning) {
      ...
      // Simulate NFC scan - would use actual NFC API in production
      const simulateScan = async () => {
        try {
          // Using a mock NFC ID
          const patient = await simulateNfcScan('nfc-12345');
          
          if (patient) {
            setScanStatus('success');
            setMessage(`Carte détectée pour ${patient.name}`);
